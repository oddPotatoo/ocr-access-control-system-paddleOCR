...
from database import insert_vehicle_entry
import uuid
import os
...
def extract_name(data, id_type):
    ...
    if id_type == "Driver's License":
        name_match = re.search(
            r"Last\s*Name\s*[,\.\-]?\s*First\s*Name\s*[,\.\-]?\s*Middle\s*Name\s*([A-Z]+)\s*[,.]?\s*([A-Z]+(?:\s+[A-Z]+)*)\s*([A-Z]*)\s*(?=Nationality|Nationallty|Noticnalty|Date\s*of\s*Birth|Weight|Height|Sex|$)",
            data,
            re.I
        )
        if name_match:
...
def process_image_with_ocr(image_path):
    ...
    return extracted_name, id_type, registration_number, qr_code_path, entry_id
